...
/**
*@desc The navigation api namespace.
*/
class api_navigation {

    /**
    * @desc Displays an informative page that display information for http status code xyz.
    */
    public static function show_xyz($xyz, $name = null, $message = null) {
        $xyz = intval($xyz);
        if ($xyz == 404)
        ...
        api_navigation::info("$xyz - $desc", $info);
    }
    ...
    */
    public static function show_not_modified() {
        if (headers_sent())
            throw new Exception("Cannot return not modified if headers are sent!");
        api_misc::ob_reset();
    ...
    */
    public static function show_404() {
        ...
        $msg = "<p>" . __("The page you requested does not exist on this server: %s%s%s", '<br /><br /><span style="font-family: monospace;">', 'http://' . @$_SERVER['HTTP_HOST'] . @$_SERVER['REQUEST_URI'], '</span><br />');
        $rooturl = CONFIG::$rooturl;
        if (!empty($_SERVER['HTTP_REFERER'])) {
            $ref = $_SERVER['HTTP_REFERER'];
            if (@substr($ref,0,strlen($rooturl)) == $rooturl) {
                $msg .= "<p>" . __("We noticed that you came here trough one of our own links, and you might want to report this issue to the site administrator and help us fix it!") . "</p>";
            } else {
                $msg .= "<p>" . __("You appear to have reached this site trough a broken link on this location:") . "<a href=\"$ref\">$ref</a><br />" . __("You might want to notify the owners of the broken link that the link is now broken.") . "</p>";
            }
        }
        $msg .= "<p><a href=\"$rooturl\">" . __("Go to our main page.") . "</a></p>";
    ...
    * @return Void Does not return, exits the request when done.
    */
    public static function show_invalid($reason = null) {
        // Prefer exceptions in developer mode.
        if (devmode)
            throw new Exception("Invalid request raised, reason specified: '$reason'");
        api_misc::ob_reset();
        if (!headers_sent()) {
    ...
    /**
    * @desc Displays given information Vector styled.
    * @param String $topic Topic of information page.
    * @param String $body HTML formated body of information page.
    * @return Void Does not return, exits the request when done.
    */
    public static function info($topic = "VectorSP Information Page", $body) {
        $rooturl = CONFIG::$rooturl;
        if (devmode && defined("bypassbtn")) $body .= bypassbtn;
        api_misc::ob_reset();
        restore_workdir();
    ...
    */
    public static function go_back() {
        ...
        if (substr($referer, 0, $blen) == CONFIG::$rooturl)
            api_navigation::redirect($referer);
        else
    ...
    */
    public static function current_url($clean_get = false) {
        if ($clean_get) {
    ...
    /**
    * @desc Signals a location redirect to client browser, useful to reload Vector on new conditions.
    */
    public static function reflexive_redirect() {
        api_navigation::redirect(api_navigation::current_url());
        exit();
    }
    ...
    */
    public static function make_local_url($path, $get = null) {
        if ($path[0] != '/')
    ...
    */
    public static function make_url($url, $get = null) {
        if (strpos($url, "?") !== FALSE) {
    ...
    */
    static function send_file($filepath, $mime = 'application/octet-stream', $filename = null) {
        if (headers_sent())
