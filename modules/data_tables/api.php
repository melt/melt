...
function include_dt() {
    ...
    if ($included)
        return;
    if (!\nmvc\core\module_loaded("Jquery"))
        trigger_error("DataTables requires the Jquery module!", \E_USER_ERROR);
    \nmvc\View::render("/data_tables/include", null, false, true);
    $included = true;
}
 ...
 * @param string $model_name Shortened classname of the model that should be listed. (The Xyz in nmvc\XyzModel)
...
function list_model($model_name, $view_url, $insert_url = null, $where = null, $columns = null, $delete_batch_operation = false, $batch_operations = array()) {
    $class_name = "nmvc\\" . $model_name;
    if (!class_exists($class_name) || !is_subclass_of($class_name, 'nmvc\Model'))
        trigger_error("$class_name is not a valid model!", \E_USER_ERROR);
    if ($columns === null) {
        $interfaces = class_implements($class_name);
        if (!isset($interfaces['nmvc\data_tables\DataTablesListable']))
    ...
    $column_names = $class_name::getColumnNames();
    ...
    $controller->batch_data = \nmvc\string\simple_crypt(serialize(array($class_name, $where)));
    $controller->id = "dt" . \nmvc\string\random_hex_str(10);
    \nmvc\View::render("/data_tables/table", $controller, false, true);
...
function get_batch_op_data() {
    if (!isset($_POST['data']) || !isset($_POST['ids']))
        \nmvc\request\show_invalid("Data missing");
    $data = $_POST['data'];
