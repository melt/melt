...
class JqueryModule extends \nmvc\CoreModule {
    public static function beforeRequestProcess() {
        ...
        if (is_string($ui_theme) && strlen($ui_theme) > 0)
            \nmvc\View::render("/jquery/include_ui", array("theme" => $ui_theme), false, true);
        if (config\INCLUDE_JQUERY_ALERTS)
            \nmvc\View::render("/jquery/include_alerts", null, false, true);
        ...
        if (config\INCLUDE_JQUERY_AUTOCOMPLETE)
            \nmvc\View::render("/jquery/include_autocomplete", null, false, true);
        if (config\INCLUDE_JQUERY_AUTORESIZE)
            \nmvc\View::render("/jquery/include_autoresize", null, false, true);
        if (config\INCLUDE_JQUERY_COOKIE)
            \nmvc\View::render("/jquery/include_cookie", null, false, true);
        if (config\INCLUDE_JQUERY_FORM)
            \nmvc\View::render("/jquery/include_form", null, false, true);
        if (config\INCLUDE_JQUERY_RESIZE)
            \nmvc\View::render("/jquery/include_resize", null, false, true);
        if (config\INCLUDE_JQUERY_HOTKEYS)
            \nmvc\View::render("/jquery/include_hotkeys", null, false, true);
        if (config\INCLUDE_JQUERY_JSTREE) {
            \nmvc\View::render("/jquery/include_jstree", null, false, true);
    ...
    public static function getInfo() {
        ...
        . "<li>jquery-form 2.47 - http://malsup.com/jquery/form/</li>"
